#!/bin/bash
# My Crew 초기 설정 스크립트 (Linux / WSL2 Ubuntu)
# 사용법: bash setup-linux.sh
# 주의: 이 스크립트는 setup-windows.ps1에서도 호출됩니다.
#       브라우저 자동 열기는 PowerShell이 처리하므로 여기서는 하지 않습니다.
...
echo ""
echo "========================================="
echo "  My Crew - AI 에이전트 팀 설정 (Linux)"
echo "========================================="
echo ""
...
# 1. Node.js 확인 (없으면 NodeSource로 설치)
if command -v node &>/dev/null; then
  ...
  echo "📦 Node.js 20.x 설치 중 (NodeSource)..."
...
# 패키지 배포본 기본값: Google SSO 온보딩 + 스토어 관리형(앱 0개 시작).
MYCREW_REQUIRE_SSO=1
MYCREW_STORE_MANAGED=1
MYCREW_STORE_URL=https://store.dooitspace.com
...
# 기존 서버 종료 (포트 충돌 방지) — fuser 우선, 없으면 ss 폴백
if command -v fuser &>/dev/null; then
  fuser -k ${PORT}/tcp 2>/dev/null || true
elif command -v ss &>/dev/null; then
  PID=$(ss -ltnp "sport = :${PORT}" 2>/dev/null | grep -oP 'pid=\K\d+' | head -1)
  [ -n "$PID" ] && kill "$PID" 2>/dev/null || true
fi
sleep 1
nohup npx tsx src/index.ts > /tmp/teamLGS.log 2>&1 &

# 브라우저 자동 열기는 호출자(PowerShell 또는 사용자)가 처리합니다.

echo "✅ 서버가 시작되었습니다. (백그라운드 실행 중)"
echo ""
echo "📋 브라우저에서 http://localhost:${PORT} 를 열면 온보딩 화면이 나옵니다."
echo "   회사명, 사용자 이름, 비서 이름을 입력하고 '시작하기'를 눌러주세요."
echo ""
echo "💡 이 터미널 창은 닫아도 됩니다. 서버는 계속 실행됩니다."
echo "   서버 종료: fuser -k ${PORT}/tcp"
