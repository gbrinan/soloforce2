...
import {
  Box, Button, Card, Checkbox, Divider, Group, Stack, Text, TextInput, Title,
} from "@mantine/core";
...
export default function InputScreen({ onJobCreated, inline }: Props) {
  const { t } = useI18n();
  ...
  return (
    <Stack gap="lg" align="center" style={{ padding: "var(--space-6)" }}>
      {/* Header bar — locale toggle removed: host shell controls language globally. */}
      <Group justify="flex-start" w="100%" maw={800}>
        <Title order={2} c="var(--text-primary)" fz="var(--font-l)">
          {t("app.name")}
        </Title>
      </Group>
      ...
      <Divider w="100%" maw={600} label={t("input.or")} labelPosition="center" />
