...
- [x] 타입 검사·18개 테스트·빌드 통과
- [ ] 실제 Google Drive/GAS 설치와 녹음 감지
- [ ] 실제 Groq 전사 종단 검증
- [ ] 운영 서버 배포 및 Soloforce2 실제 인스턴스 가져오기

마지막 세 항목은 서버·계정 설정과 실제 녹음이 필요하다. 외부 API 시험 대신 성공처럼 보이는 데모 결과를 반환하는 운영 모드는 없다. 예제 생성에서만 명시적으로 가상 응답을 사용한다.

검증 명령: `bun run check`, `bun test`, `bun run build`, `bun src/manual-qa.ts`. 마지막 명령은 가상 응답으로 실제 로컬 HTTP 경로를 실행하고 예제 파일을 갱신한다.

## 제한과 다음 담당자의 작업
단일 소유자·단일 서버 프로세스용 SQLite 서비스다. 별도 조직별 ACL, 대용량 음성 분할, 의미 기반 검색 평가, 동명이인 해소, 그래프 DB 직접 동기화는 포함하지 않는다. Gemini Files API를 사용하며 업로드는200MiB, Groq는25MB 이하. 전체 전문은 SQLite와 Markdown API에 보존한다. 검색은 최대 8개 관련 자료의 키워드 일치 기반으로 시작하며 한국어 품질은 업무 데이터로 평가해야 한다.

현재 인프라 담당자가 [설치 절차](docs/setup.md)의 미제공 항목을 설정하고 실녹음 한 건을 검증하면 운영 인수로 이어갈 수 있다. Soloforce2 연결은 [저장소 설치 안내](../../docs/meeting-memory.md)를 따른다.
